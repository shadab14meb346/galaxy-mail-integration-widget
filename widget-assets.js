...
const scanGumroadProductsLinkOnThePage = () => {
	const gumroadLink = "https://app.galaxymail.co";
...
const openGalaxyModal = () => {
	const iFrame = document.getElementById(iframeId);
	const loader = document.getElementById(loaderId);
	iFrame.src = "https://app.galaxymail.co/connect-wallet?step=1";
	const iframeContainer = document.getElementById(iframeContainerId);
	iframeContainer.style.display = "flex";
	iframeContainer.style.justifyContent = "center";
	iframeContainer.style.alignItems = "center";
...
const closeGalaxyModal = () => {
	const loader = document.getElementById(loaderId);
	const iframeContainer = document.getElementById(iframeContainerId);
	iframeContainer.style.display = "none";
	loader.style.display = "none";
};
const checkWalletAddressStatus = async (walletAddress) => {
	const response = await fetch(
		`https://api.galaxymail.co/is-wallet-present/${walletAddress}`
	);
...
scanGumroadProductsLinkOnThePage();
window.onmessage = (event) => {
	if (event.data === "WALLET_CONNECTED") {
		closeGalaxyModal();
		return;
	}
	if (event.data?.type === "WALLET_CONNECTED") {
		checkWalletAddressStatus(event.data?.account);
	}
};
